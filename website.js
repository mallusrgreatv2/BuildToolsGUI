...
document.getElementById("build").onclick = async function () {
  if (window.downloadPath) {
    ...
    await download(
      ...
      window.downloadPath.replace(/\|_/g, "/")
    );
    ...
    var child = spawn(
      ...
      [
        "-jar",
        window.downloadPath.replace(/\|_/g, "/") + "/BuildTools.jar",
        "--rev",
    ...
    child.stdout.on("data", function (data) {
      if (data.toString().includes("Success! Everything completed")) {
        ...
        document
          ...
          .classList.remove("siimple-btn--disabled");
        document
          .getElementById("progress")
          .classList.add("siimple-progress--success");
      } else {
        document.getElementById("output").innerHTML =
          document.getElementById("output").innerHTML + "\n" + data.toString();
      }
      ...
      if (data.toString().includes("Patching with Block.patch"))
        document.getElementById("progress").style.width = "25%";
      if (
        data
          .toString()
          .includes(
            "Extracted: workdecompile-ee3ecae0classes\netminecraftserverDragonControllerHover.class"
          )
      )
        document.getElementById("progress").style.width = "25%";
      ...
      if (
        data
          .toString()
          .includes(
            "Extracted: workdecompile-ee3ecae0classes\netminecraftserverEnchantmentSweeping.class"
          )
      )
      ...
      if (
        data
          .toString()
          .includes(
            "Extracted: workdecompile-ee3ecae0classes\netminecraftserverTileEntityCommand$1.class"
          )
      )
      ...
      if (
        data
          .toString()
          .includes("Patching with PathfinderGoalNearestAttackableTarget.patch")
      )
      ...
      if (
        data
          .toString()
          .includes(
            "Extracted: workdecompile-ee3ecae0classes\netminecraftserverArgumentVectorPosition.class"
          )
      )
        document.getElementById("progress").style.width = "50%";

      if (data.toString().includes("Applying patches to Spigot-API"))
        document.getElementById("progress").style.width = "60%";
      if (
        data
          .toString()
          .includes("Decompiling class net/minecraft/server/WorldMap")
      )
        document.getElementById("progress").style.width = "60%";

      if (data.toString().includes("Applying: Plug WorldMap Memory Leak"))
        document.getElementById("progress").style.width = "75%";
      if (
        data
          .toString()
          .includes("INFO:  Decompiling class net/minecraft/server/TagsItem")
      )
        document.getElementById("progress").style.width = "75%";

      if (data.toString().includes("*** Spigot patches applied!"))
        document.getElementById("progress").style.width = "80%";
      if (
        data
          .toString()
          .includes(
            "INFO:  Decompiling class net/minecraft/server/PacketPlayInVehicleMove"
          )
      )
        document.getElementById("progress").style.width = "80%";
...
function changeColorTheme() {
  if (!nativeTheme.shouldUseDarkColors) {
    ...
    document
      .getElementById("progressbar")
      .classList.add("siimple-progress--dark");
  }
}
